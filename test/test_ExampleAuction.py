...
class TestExampleAuction(unittest.TestCase):
    ...
    def test_auctionWorkflow(self):
        ...
        self.assertTrue(self.auction_contract)
        self.assertTrue(self.erc721_contract)
        ...
        print(dir(self.erc721_contract))
        print(self.erc721_contract.address)
        #self.erc721_contract.addMinter(rec_hex(CONTRACT_OWNER_ADDRESS))
        self.assertTrue(self.erc721_contract.isMinter(CONTRACT_OWNER_ADDRESS))
        #self.assertTrue(self.erc721_contract.mint(rec_hex(CONTRACT_OWNER_ADDRESS), TOKEN_ID, sender=CONTRACT_OWNER_PRIVATE_KEY))
        self.chain.mine(1)
        ...
        self.assertEqual(COMMIT_PERIOD_LENGTH, self.auction_contract.commitPeriodLength())
        self.assertEqual(REVEAL_PERIOD_LENGTH, self.auction_contract.revealPeriodLength())
        self.assertEqual(starting_block_height + COMMIT_PERIOD_LENGTH + REVEAL_PERIOD_LENGTH, self.auction_contract.auctionEndBlock())
        ...
        # session_dataAlice = self.auction_contract.getCommitState(rec_bin(commitAlice))
        ...
        # finished_boolAlice = self.auction_contract.finished(rec_bin(commitAlice))
        # self.assertFalse(
        #     finished_boolAlice,
        ...
        # session_dataBob = self.auction_contract.getCommitState(rec_bin(commitBob))
        # self.assertListEqual(session_dataBob,
        #         [SOLIDITY_NULL_INITIALVAL, SOLIDITY_NULL_INITIALVAL, SOLIDITY_NULL_INITIALVAL, SOLIDITY_NULL_INITIALVAL])

        # finished_boolBob = self.auction_contract.finished(rec_bin(commitBob))
        # self.assertFalse(
        #     finished_boolBob,
        #     "The contract should not be finished before it's even begun.")
        ...
        # session_dataCharlie = self.auction_contract.getCommitState(rec_bin(commitCharlie))
        # self.assertListEqual(session_dataCharlie,
        #         [SOLIDITY_NULL_INITIALVAL, SOLIDITY_NULL_INITIALVAL, SOLIDITY_NULL_INITIALVAL, SOLIDITY_NULL_INITIALVAL])

        # finished_boolCharlie = self.auction_contract.finished(rec_bin(commitCharlie))
        # self.assertFalse(
        ...
        # self.auction_contract.revealBid(
        ...
        # bidListItemBob = self.auction_contract.BidList(1)
        ...
        # bidListItemCharlie = self.auction_contract.BidList(2)
        ...
        # self.assertFalse(self.auction_contract.auctionIsOver())
        ...
        # self.auction_contract.endauction(sender=RANDO_ADDRESS_PRIVATE_KEY)

        # ##
        # ## CHECK STATE NOW THAT AUCTION IS OVER

        # self.assertTrue(self.auction_contract.auctionIsOver())
        # self.assertEqual(rec_hex(CHARLIE_ADDRESS), self.auction_contract.dogieOwner())
