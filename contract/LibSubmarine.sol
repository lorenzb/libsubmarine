...
contract LibSubmarine is ProvethVerifier {
    ...
    uint256 public revealDeposit;   /* TODO: check if can be smaller than uint256 */
    ...
    event Unlocked(bytes32 indexed _sessionId, uint256 _commitValue);
    event Revealed(bytes32 indexed _sessionId, uint256 _commitValue, bytes _data, bytes32 _witness, uint32 _commitBlock, uint256 _commitIndex);
    event Slashed(bytes32 indexed _sessionId, uint8 _result, address indexed _sender, address indexed _submarine, uint256 _slashAmount, bytes _proofBlob, bytes _unsignedCommitTx);
    ...
    constructor(uint256 _revealDeposit, uint32 _challengePeriod) public {
    ...
    function isFinalizable(bytes32 _sessionId) public view returns (bool finalized, uint256 commitValue, bytes data) {
        if (block.number > sessions[_sessionId].revealBlock.add(challengePeriod) && sessions[_sessionId].unlocked && sessions[_sessionId].revealBlock > 0) {
            return (true, sessions[_sessionId].commitValue, sessions[_sessionId].data);
        }
        return (false, 0, "");
    }
     ...
     * @dev Must be called within 256 blocks of the commit transaction to obtain the correct blockhash.
    ...
    function reveal(
        ...
        bytes32 sessionId = keccak256(abi.encodePacked(msg.sender, address(this), _commitValue, _data, _witness, _gasPrice, _gasLimit)); //implicitly checks msg.sender is A to generate valid sessionId
        require(msg.value >= revealDeposit, 'Reveal deposit not provided');
    ...
    function unlock(bytes32 _sessionId) public payable {
        require(sessions[_sessionId].revealBlock > 0 && !sessions[_sessionId].unlocked, 'The tx is already unlocked, or not yet revealed');
    ...
    /**
     * @notice Allows anyone to submit a proof that the reveal tx does not match the commit tx.
     * @notice If the proof is valid, this means the user cheated and the challenger retreives the funds. (TODO: deal with case where funds are locked if challenge submitted before unlock)
     * @param _sessionId Hash of the session instance representing the commit/reveal transaction
    ...
    function challenge(bytes32 _sessionId, bytes _proofBlob, bytes _unsignedCommitTx) public {
        require(block.number < sessions[_sessionId].revealBlock.add(challengePeriod), 'Challenge period is not active');
        ...
        address submarine = ecrecover(unsignedTx.sigHash, vee, keccak256(abi.encodePacked(_sessionId, byte(1))), keccak256(abi.encodePacked(_sessionId, byte(0)))); //ecrecover(bytes32 hash, uint8 v, bytes32 r, bytes32 s) returns (address)
        ...
        */
        if (mpProof.index == sessions[_sessionId].commitIndex
            ...
            uint256 slashAmount = sessions[_sessionId].commitValue.add(revealDeposit);
    ...
    /**
     * @notice Allows the dApp to retreive the commit value and the reveal deposit.
    ...
    function finalize(bytes32 _sessionId) public returns (uint256 commitValue, bytes memory data) {
        ...
        require(block.number > sessions[_sessionId].revealBlock.add(challengePeriod) && sessions[_sessionId].unlocked, 'The challenge period is not over, the tx was not unlocked, or the session was slashed');
        commitValue = sessions[_sessionId].commitValue;
        data = sessions[_sessionId].data;
        sessions[_sessionId].dappAddress.transfer(commitValue.add(revealDeposit)); /* TODO: check if it is possible to return the deposit to the user directly */
        emit Finalized(_sessionId, msg.sender, commitValue, revealDeposit, data);
        delete sessions[_sessionId];
    }
}
